...
# offline-plugin  <Badge text='v 5.0.7' />
...
本插件旨在为`webpack`项目提供离线体验。它利用了`ServiceWorker`和`AppCache`作为后台备份。只需要在您的`webpack.config.js`中引入这个插件，并在客户端脚本中包含相应的`runtime(运行时)`。通过缓存所有(或部分) `webpack`输出的资源，您的项目就可以离线了。
